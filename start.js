...
bot.on('message', function (user, userID, channelID, message, evt) {
    ...
    if (userID == "365975655608745985") { return }
    if ((channelID == "698874728932180018" || channelID == "700717192072921088") == false) { return }
    if (user == "Trilliax") { return }
    if (message == "p!hint") { return }

    console.log("Message from: " + user + " - " + message)

    var messageComponenets = message.split(' ')
    if (messageComponenets.length != 2) {
        bot.sendMessage({
            to: channelID,
            message: 'Filthy!'
        });
    ...
    var commandPassed = message.split(' ')[0]
    var pokemonSpecified = message.split(' ')[1].toUpperCase()

    if (allPokemon.includes(pokemonSpecified) && (commandPassed == "p!catch" || commandPassed == "p!hint")) { 
        bot.sendMessage({
            to: channelID,
            message: 'Legit Pokemon'
        });
        return 
    }
