...
var originalPokemon = ["BULBASAUR","IVYSAUR","VENUSAUR","CHARMANDER","CHARMELEON","CHARIZARD","SQUIRTLE","WARTORTLE","BLASTOISE","CATERPIE","METAPOD","BUTTERFREE","WEEDLE","KAKUNA","BEEDRILL","PIDGEY","PIDGEOTTO","PIDGEOT","RATTATA","RATICATE","SPEAROW","FEAROW","EKANS","ARBOK","PIKACHU","RAICHU","SANDSHREW","SANDSLASH","NIDORAN♀","NIDORINA","NIDOQUEEN","NIDORAN♂","NIDORINO","NIDOKING","CLEFAIRY","CLEFABLE","VULPIX","NINETALES","JIGGLYPUFF","WIGGLYTUFF","ZUBAT","GOLBAT","ODDISH","GLOOM","VILEPLUME","PARAS","PARASECT","VENONAT","VENOMOTH","DIGLETT","DUGTRIO","MEOWTH","PERSIAN","PSYDUCK","GOLDUCK","MANKEY","PRIMEAPE","GROWLITHE","ARCANINE","POLIWAG","POLIWHIRL","POLIWRATH","ABRA","KADABRA","ALAKAZAM","MACHOP","MACHOKE","MACHAMP","BELLSPROUT","WEEPINBELL","VICTREEBEL","TENTACOOL","TENTACRUEL","GEODUDE","GRAVELER","GOLEM","PONYTA","RAPIDASH","SLOWPOKE","SLOWBRO","MAGNEMITE","MAGNETON","FARFETCH’D","DODUO","DODRIO","SEEL","DEWGONG","GRIMER","MUK","SHELLDER","CLOYSTER","GASTLY","HAUNTER","GENGAR","ONIX","DROWZEE","HYPNO","KRABBY","KINGLER","VOLTORB","ELECTRODE","EXEGGCUTE","EXEGGUTOR","CUBONE","MAROWAK","HITMONLEE","HITMONCHAN","LICKITUNG","KOFFING","WEEZING","RHYHORN","RHYDON","CHANSEY","TANGELA","KANGASKHAN","HORSEA","SEADRA","GOLDEEN","SEAKING","STARYU","STARMIE","MR. MIME","SCYTHER","JYNX","ELECTABUZZ","MAGMAR","PINSIR","TAUROS","MAGIKARP","GYARADOS","LAPRAS","DITTO","EEVEE","VAPOREON","JOLTEON","FLAREON","PORYGON","OMANYTE","OMASTAR","KABUTO","KABUTOPS","AERODACTYL","SNORLAX","ARTICUNO","ZAPDOS","MOLTRES","DRATINI","DRAGONAIR","DRAGONITE","MEWTWO","MEW"]
...
function changeSpawnTimerTo(newSpawntimer) {
...
function getLeaderboard(channelID) {
    ...
    var allPlayers = Object.keys(currentQuizPlayers)
    console.log(allPlayers)

    var players = new Array();

    for (var player in currentQuizPlayers) {
        players.push({"trainerName" : currentQuizPlayers[player].trainerName, "score": currentQuizPlayers[player].score})
    }
    ...
    var playerScores = players.map(function(player, index) {
        return (index + 1) + ": " + player.trainerName + " - " + player.score;
    });
    console.log(playerScores)

    bot.sendMessage({
        to: channelID,
        message: playerScores.join("\n")
    });
}
...
function getUserScore(userID, channelID) {
    ...
    bot.sendMessage({
        to: channelID,
        message: "<@" + userID + "> your current score is " + currentQuizPlayers[userID].score
    });
}
...
function handlePokemonGuess(channelID, userID, pokemonGuessed) {
    ...
    updatePlayerScore(userID, player.score)
    setUpSpawnTimer()

    bot.sendMessage({ to: channelID, message: "correct <@" + userID + "> your score is " + player.score });
}

function updatePlayerScore(userID, newScore) {
    ...
    fs.writeFile('../trainers.json', stringifiedTrainers, (err) => {
...
function spawnRandomPokemon(channelID) {
    ...
    var randomPokemonID = Math.floor(Math.random() * Math.floor(originalPokemon.length - 1))
    pokemonSpawned = originalPokemon[randomPokemonID - 1];

    setUpGuessTimer()
    bot.sendMessage({
        to: channelID,
        message: "https://pokeres.bastionbot.org/images/pokemon/" + randomPokemonID + ".png"
    });
}
...
function createTrainerFile() {
    ...
    fs.writeFile('../trainers.json', data, (err) => {
        console.log('Data written to file');
    });
